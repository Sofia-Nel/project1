...
```{r tai, echo=FALSE}
kbl(tabela1,caption = "Gross income - Biedronka - values in kPLN") %>%
...
## Summary tables (do 2 or 3 examples)
...
```
 ...
 Gross income is not significant different in cities.
...
Rating is not significant different by customer type. 
...
```
Quantity (number of products purchased by the customer) is significant different by gender. Women buy more products on average than men.
...
```
 ...
 Unit price of each product is not significant different by product line. 
...
```
Crossed out values indicates no linear relationship. 1 indicates a perfect positive linear relationship.There is a positive correlation (0.63) between unit price and tax 5%, total, cogs and gross income. This suggests that, for example, as the unit price increases, the 5% tax also increases. There is a positive correlation (0.71) between quantity and tax 5%, total, cogs and gross income. This suggests that, for example, as the quantity (the number of products purchased by the customer) increases, the 5% tax also increases.
